...
└── AppDelegate (NSApplicationDelegate)
    ├── DynamicNotchSkyLightWindow (NSPanel) — one per screen
    │   └── ContentView (SwiftUI root)
    │       ├── NotchShape — the pill/bezel shape
    │       ├── [closed] MusicLiveActivity / InlineHUD / BatteryNotif / FaceAnimation
    │       ├── [open] NotchHeader (tabs + battery icon + camera toggle + gear)
    │       └── [open] NotchHomeView  ← or ←  ShelfView
    └── NotchViewCoordinator.shared (singleton — global state)
...
| File | Role |
|------|------|
| `DynamicNotch/DynamicNotchApp.swift` | `@main` struct + `AppDelegate`. Creates windows, handles screen changes, keyboard shortcuts, onboarding, lock-screen events. |
| `DynamicNotch/components/Notch/DynamicNotchSkyLightWindow.swift` | Custom `NSPanel` subclass. Sits at `level = .mainMenu + 3`, forces `.darkAqua`. Uses private SkyLight framework to optionally appear on the lock screen. |
| `DynamicNotch/components/Notch/DynamicNotchWindow.swift` | Older `NSPanel` base class (no SkyLight). Not used for the main window anymore but kept. |
| `DynamicNotch/sizing/matters.swift` | All sizing constants. **The single source of truth for dimensions.** |

### State & Coordination

| File | Role |
|------|------|
| `DynamicNotch/models/NotchViewModel.swift` | Per-screen ViewModel. Owns `notchState` (`.open`/`.closed`), notch size, drag/drop targeting flags, camera expanded flag. One instance per physical screen. |
| `DynamicNotch/NotchViewCoordinator.swift` | `@MainActor` singleton. Owns `currentView` (`.home`/`.shelf`), `sneakPeek`, `expandingView`, screen UUID, first-launch state. |
| `DynamicNotch/models/Constants.swift` | **All `Defaults.Keys`.** Every user-facing setting lives here. Also defines many supporting enums. |
| `DynamicNotch/enums/generic.swift` | Core domain enums: `NotchState`, `NotchViews`, `ContentType`, `SettingsEnum`, `DownloadIndicatorStyle`, `SliderColorEnum`, etc. |

### UI — Notch Views

| File | Role |
|------|------|
| `DynamicNotch/ContentView.swift` | Root SwiftUI view. Handles shape, animations, hover, gestures (swipe to open/close), drop zones, `NotchLayout()` builder function. |
| `DynamicNotch/components/Notch/NotchShape.swift` | Custom `Shape` that draws the notch pill with animatable top/bottom corner radii. |
| `DynamicNotch/components/Notch/NotchHeader.swift` | The header row rendered inside the open notch: tab selector (left), notch chip overlay (centre), battery/camera/settings icons (right). |
| `DynamicNotch/components/Notch/NotchHomeView.swift` | The "Home" tab content: `MusicPlayerView` → `AlbumArtView` + `MusicControlsView` (slot toolbar, slider, lyrics, song info). Also hosts `CalendarView` and `CameraPreviewView`. |
| `DynamicNotch/components/Shelf/Views/ShelfView.swift` | The "Shelf" tab. Drag-and-drop file/URL tray with Quick Look. |

### UI — Tabs

| File | Role |
|------|------|
| `DynamicNotch/components/Tabs/TabSelectionView.swift` | Tab bar in the header. The `tabs` array at the top lists all tab models. Add a new tab here and in `NotchViews`. |
| `DynamicNotch/components/Tabs/TabButton.swift` | Individual tab button. |

### UI — Live Activities (Closed State)

| File | Role |
|------|------|
| `DynamicNotch/components/Live activities/LiveActivityModifier.swift` | Helper modifier for the mini live-activity strip. |
| `DynamicNotch/components/Live activities/SystemEventIndicatorModifier.swift` | Pops out next to notch for volume/brightness HUD. |
| `DynamicNotch/components/Live activities/InlineHUD.swift` | Inline mode HUD (inside the notch pill). |
| `DynamicNotch/components/Live activities/NotchBattery.swift` | Battery icon view. |
| `DynamicNotch/components/Live activities/MarqueeTextView.swift` | `MarqueeText` — scrolling text for tight spaces. Use this for any text that might overflow. |
| `DynamicNotch/components/Live activities/OpenNotchHUD.swift` | HUD shown inside the open notch header area. |
| `DynamicNotch/components/Live activities/DownloadView.swift` | Download progress indicator. |

### Media Controllers

| File | Role |
|------|------|
| `DynamicNotch/MediaControllers/MediaControllerProtocol.swift` | Protocol every media controller must conform to. |
| `DynamicNotch/MediaControllers/NowPlayingController.swift` | macOS Now Playing API. May be deprecated on newer macOS. |
| `DynamicNotch/MediaControllers/AppleMusicController.swift` | AppleScript bridge to Music.app. |
| `DynamicNotch/MediaControllers/SpotifyController.swift` | AppleScript bridge to Spotify.app. |
| `DynamicNotch/MediaControllers/YouTube Music Controller/` | Local YouTube Music app API. |
| `DynamicNotch/managers/MusicManager.swift` | Singleton that owns the active controller, publishes all playback state (`songTitle`, `albumArt`, `isPlaying`, etc.), handles artwork, lyrics, sneak peek. |
| `DynamicNotch/models/PlaybackState.swift` | Plain struct carrying a snapshot of playback state from any controller. |
| `DynamicNotch/models/MusicControlButton.swift` | Enum of all control button types (shuffle, previous, play/pause, etc.) with label/icon metadata. The user can configure which slots show which buttons. |

### Managers

| File | Role |
|------|------|
| `DynamicNotch/managers/BatteryActivityManager.swift` | Battery level + charging state. |
| `DynamicNotch/managers/BrightnessManager.swift` | Read/set screen brightness. |
| `DynamicNotch/managers/VolumeManager.swift` | Read/set system volume. |
| `DynamicNotch/managers/CalendarManager.swift` | Fetches EventKit events/reminders. |
| `DynamicNotch/managers/WebcamManager.swift` | Camera session management. |
| `DynamicNotch/managers/NotchSpaceManager.swift` | Controls which Mission Control space the window appears in. |
| `DynamicNotch/managers/ImageService.swift` | Async image helpers. |

### Reusable UI Components

| File | Role |
|------|------|
| `DynamicNotch/components/HoverButton.swift` | **The standard media-control button.** Capsule bg appears on hover. Use this for any icon button in the notch. |
| `DynamicNotch/extensions/ActionBar.swift` | View modifier that adds a bottom action bar (divider + buttons). |
| `DynamicNotch/extensions/ConditionalModifier.swift` | `.conditionalModifier(bool) { view in ... }` helper. |
| `DynamicNotch/extensions/Button+Bouncing.swift` | Bouncing press animation for buttons. |
| `DynamicNotch/extensions/Color+AccentColor.swift` | `.effectiveAccent` — respects custom accent color setting. |
| `DynamicNotch/extensions/MouseTracker.swift` | Global mouse tracking without accessibility permissions. |
| `DynamicNotch/components/LottieView.swift` | Lottie animation wrapper. |
| `DynamicNotch/components/Music/MusicVisualizer.swift` | Audio spectrum bars. |

### Settings

| File | Role |
|------|------|
| `DynamicNotch/components/Settings/SettingsView.swift` | All settings UI. One large file with a `NavigationSplitView` and individual structs per section (GeneralSettings, Appearance, Media, CalendarSettings, HUD, Charge, Shelf, Shortcuts, Advanced, About). **Add new settings sections here.** |
| `DynamicNotch/components/Settings/SettingsWindowController.swift` | Singleton that manages the settings NSWindow lifecycle. Call `SettingsWindowController.shared.showWindow()` to open settings. |
| `DynamicNotch/components/Settings/MusicSlotConfigurationView.swift` | Drag-and-drop music control slot editor. |
...
[System event: volume changed] → VolumeManager
                                      ↓
                                  NotchViewCoordinator.toggleSneakPeek(status: true, type: .volume, value: ...)
                                      ↓ @Published sneakPeek
...
1. Add any new state to `NotchViewCoordinator` (a `@Published` bool, or a new `SneakContentType` case).
...
1. **`NotchViewCoordinator.swift`** — Add a case to `SneakContentType`:
   ```swift
...
2. Call `NotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .myEvent, value: someFloat)` from your manager.

3. **`components/Live activities/SystemEventIndicatorModifier.swift`** — Handle the new type in the icon/label rendering.

4. **`NotchHeader.swift`** — Update `isHUDType(_:)` if it should show in the open-notch HUD style.
...
| `DynamicNotchApp.swift` | AppDelegate is complex — handles multi-display, screen lock, screen config changes. Test any change across single and multiple displays. |
| `components/Notch/DynamicNotchSkyLightWindow.swift` | `level = .mainMenu + 3` keeps the notch above full-screen apps. If this changes the notch disappears under Spotlight etc. |
...
| `NotchViewCoordinator.swift` — `@AppStorage` keys | Same issue — `"firstLaunch"`, `"showWhatsNew"`, `"openLastTabByDefault"` etc. are serialised; renaming them breaks data continuity. |
...
```swift
NotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: 0.75)
```
...
vm.open()
// or via keyboard shortcut callback already wired in DynamicNotchApp.swift
```

**Adding a button to the notch header (right side):**
Edit `NotchHeader.swift` inside the `HStack(spacing: 4)` block (the trailing side). Follow the existing pattern — `Capsule().fill(.black).frame(30, 30).overlay { Image(systemName: ...) }` wrapped in a `Button`.
